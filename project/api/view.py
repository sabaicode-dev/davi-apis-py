...
class CreateProject(APIView):
    def post(self, request):
        try:
            # Extract user_cognito_id from the authenticated user
            user_cognito_id = request.user  # This comes from JWT middleware
            if not user_cognito_id:
                return Response({"error": "User not authenticated!"}, status=status.HTTP_401_UNAUTHORIZED)

            project_data = request.data
            project_data["user_cognito_id"] = user_cognito_id  # Add user_cognito_id to the project data
...
class ListProject(APIView):
    def get(self, request):
        try:
            # Extract user ID from the request (from JWT or session)
            user_cognito_id = request.user
            if not user_cognito_id:
                return Response({"error": "User not authenticated!"}, status=status.HTTP_401_UNAUTHORIZED)
            ...
            filters = {"user_cognito_id": user_cognito_id}
